...
export default function Menu() {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const itemName =
      lang === 'ar'
        ? item.nameAr || item.nameFr || item.name
        : item.nameFr || item.name;

    const itemDesc =
      lang === 'ar'
        ? item.descriptionAr || item.descriptionFr || item.description
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4 max-w-2xl mx-auto">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">
            {isRTL
              ? 'اختر وجبتك المفضلة واطلبها مباشرة عبر الواتساب'
              : 'Découvrez nos délicieux plats et commandez directement via WhatsApp'}
          </p>
