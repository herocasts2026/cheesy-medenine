...
export default function Order() {
  ...
  const generateOrderNumber = async (): Promise<string> => {
    try {
      ...
      const fallback = Date.now().toString(36).toUpperCase().slice(-5);
      return `#${fallback}`;
    }
  };
    ...
    <div className="pt-24 pb-20 bg-[#FAF9F6] dark:bg-[#1a1a1a] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <p className="text-[#F6B21A] font-black uppercase tracking-wider text-sm mb-2">{t.orderOnline}</p>
          <h1 className="text-4xl sm:text-5xl font-black text-[#2C2C2C] dark:text-white">{t.yourOrder}</h1>
        </div>
              ...
              {cartItems.map(({ item, quantity }) => (
                <div key={item.id} className="flex gap-4 p-4 rounded-2xl bg-white dark:bg-[#2C2C2C] shadow-sm">
                  {item.images.length > 0 ? (
                    <img src={item.images[0]} alt={item.nameFr} className="w-20 h-20 rounded-xl object-contain p-1 flex-shrink-0" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = '/images/fallback-food.webp'; }} />
                  ) : (
              ...
              <div className="p-6 rounded-2xl bg-white dark:bg-[#2C2C2C] shadow-sm space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={isRTL ? 'العنوان الكامل في مدنين' : 'Votre adresse complète à Médenine'}
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-4 py-3 rounded-xl bg-[#FAF9F6] dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-700 text-[#2C2C2C] dark:text-white focus:border-[#F6B21A] focus:outline-none transition-colors resize-none"
                    placeholder={isRTL ? 'أي ملاحظات خاصة...' : 'Notes spéciales...'}
                  />
                </div>
                ...
                >
                  <Send size={18} />
                  {isGenerating ? (isRTL ? 'جاري التوليد...' : 'Génération...') : t.sendOrder}
                </button>
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-[80] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white dark:bg-[#1a1a1a] rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="px-6 py-5 space-y-4">
                <div className="text-center py-2">
                  ...
                  <p className="text-3xl font-black text-[#F6B21A]">{orderNumber}</p>
                </div>
                <div className="flex items-start gap-2 p-3 rounded-xl bg-[#F6B21A]/10 text-sm text-[#2C2C2C] dark:text-gray-300">
                  ...
                  <p>{isRTL
                    ? `رقم طلبك هو ${orderNumber}. يرجى الاحتفاظ بهذا الرقم. يمكنك استخدامه للاستعلام عن طلبك لدى المطعم.`
                    : `Votre numéro de commande est ${orderNumber}. Gardez ce numéro. Vous pouvez l'utiliser plus tard pour demander au restaurant l'état de votre commande.`}</p>
                </div>
              ...
              <div className="px-6 py-5 border-t border-gray-100 dark:border-gray-800">
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl bg-[#25D366] hover:bg-[#1ebe57] text-white font-black transition-all hover:scale-[1.02]"
                >
                  <Send size={18} />
                  {isRTL ? 'تأكيد وإرسال' : 'Confirmer & Envoyer'}
                </button>
              </div>
            </div>
          </div>
